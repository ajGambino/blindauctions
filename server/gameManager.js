...
class GameManager {
  ...
  placeBid(gameId, socketId, bidAmount) {
    ...
    const position = game.currentPlayer.position;
    if (position === 'QB' && user.team.QB) return { error: 'QB position already filled' };
    if (position === 'RB' && user.team.RB) return { error: 'RB position already filled' };
    if (position === 'WR' && user.team.WR.length >= 2) return { error: 'WR positions already filled' };
    if (position === 'TE' && user.team.TE) return { error: 'TE position already filled' };
